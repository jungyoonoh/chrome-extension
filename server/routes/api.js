...
// 1. 네이버 뉴스 api를 이용해 뉴스 정보 가져옴 
// 문제점 : 핫 토픽 뉴스(get results)는 가져오지 못함 
router.get(`/news`,(req,res)=>{
  const api_url = `https://openapi.naver.com/v1/search/news?query=${encodeURI(`삼성전자`)}`; //query=검색어 , sort는 정렬 순서, 기본값은 정확도 순
  const options = {
      url: api_url,
...
router.post(`/movie`,(req,res)=>{
  const api_url = `https://openapi.naver.com/v1/search/movie.json?query=${encodeURI(req.body.keyword)}`; //query=검색어 
  const options = {
      url: api_url,
      headers: {'X-Naver-Client-Id':process.env.CLIENT_ID, 'X-Naver-Client-Secret': process.env.CLIENT_SECRET}
   };
